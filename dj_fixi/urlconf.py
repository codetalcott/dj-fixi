...
Public API from 0.3.0. dj-fixi's system checks are built on this, and so is the
companion package dj-fixi-tables -- which is exactly why it lives here rather
than inside ``dj_fixi.checks``: importing it must not register system checks.
...
checks are what should report a broken URLconf; a dj-fixi traceback on top of
that would only misattribute the problem.
"""
